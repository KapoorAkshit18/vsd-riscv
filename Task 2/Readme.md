...
(one which was compiled with fast optimization) It is recommended to change the object file name for each optimization, 
`riscv64-unknown-elf-objdump -d sum_1_to_n.o | less`  
  </details>
initially for debugging we will know output or affect of each assembly code to the processors architecture registers, for that purpose we will run the below command:  
